...
//方位辨别
  Dates.orien = function(obj, pos){
    ...
    obj.style.left = Dates.elem.offsetLeft + 'px';
    obj.style.top = (Dates.elem.offsetTop + Dates.elem.offsetHeight) + 'px';
  };
...
//渲染控件骨架
  Dates.view = function(elem, options){
    ...
    if(!Dates.box){
      ...
      var _container = S('#content').children[0];
      _container.appendChild(div);
      _container.style.position = 'relative';
      /*Dates.elem.parentNode.insertBefore(div, Dates.elem.nextSibling);*/
      Dates.box = S('#'+as[0]);
      Dates.events();
      div = null;
    } else {
